...
import { Button, Card, Spinner, ErrorState, Badge, DeletionConfirmModal, cx } from '@/components/ui'
...
export default function VariantEditor({ variantId, variants, onSelectVariant, onSaved, hideVariantSelect = false }: {
  ...
  async function remove(q: Question) {
    ...
    try { await api(`/ent-trial/admin/questions/${q.id}`, { method: 'DELETE' }); await refresh(); setPendingDelete(null) }
  ...
  return <div className="space-y-5">
    <DeletionConfirmModal open={pendingDelete !== null} onClose={() => setPendingDelete(null)} onConfirm={() => { if (pendingDelete) void remove(pendingDelete) }} title="Сұрақты өшіру" description="Осы сұрақты өшіргіңіз келе ме? Қалған сұрақтардың нөмірлері өзгермейді." courseName={pendingDelete ? `№${pendingDelete.position + 1} · ${SUBJECT_NAMES[pendingDelete.subject] ?? pendingDelete.subject}` : ''} warning={actionError || undefined} isLoading={deleting} />
    {!hideVariantSelect && <label className="block space-y-2"><span className="text-sm font-semibold">Нұсқа</span>
      <select className={inputClass} value={variantId ?? ''} onChange={e => navigate(() => { onSelectVariant(Number(e.target.value) || null); setSubject('kaz_history'); setPosition(0) })}>
        <option value="">Нұсқаны таңдаңыз</option>{variants.map(v => <option key={v.id} value={v.id}>{v.title}</option>)}
      </select>
    </label>}
    {isLoading && <Spinner />}{error && <ErrorState message={error.message} />}{actionError && <ErrorState message={actionError} />}
    {data && <>
      <Card className="space-y-3 p-5">
        <div className="flex flex-wrap items-center gap-3"><Badge tone={data.validation.ready ? 'success' : 'warning'}>{data.validation.ready ? 'Дайын' : 'Толықтыру қажет'}</Badge><h2 className="text-lg font-bold">{Object.entries(data.rules).reduce((count, [key, r]) => count + r.slots.filter(s => data.items.some(q => q.subject === key && q.position === s.position)).length, 0)} / {Object.values(data.rules).reduce((n, r) => n + r.slots.length, 0)} сұрақ</h2></div>
        ...
        {!data.validation.ready && <details><summary className="cursor-pointer font-medium text-warning-foreground">Толықтыру қажет: {data.validation.issues.length} ескерту</summary>
      ...
      {rule && <>
        ...
        {question && <Button variant="danger-ghost" onClick={() => { setActionError(''); setPendingDelete(question) }}>Осы сұрақты жою</Button>}
      </>}
      {extras.length > 0 && <Card className="space-y-3 p-5"><h3 className="font-semibold">Құрылымнан тыс ескі сұрақтар</h3><p className="text-sm text-muted">Мәтіндер сақталған. Қажетті мазмұнды тиісті орынға көшіріп, содан кейін артық сұрақты жойыңыз.</p>{extras.map(q => <details key={q.id}><summary>{SUBJECT_NAMES[q.subject] || q.subject} №{q.position + 1} · ID {q.id}</summary><p className="whitespace-pre-wrap py-3">{q.prompt}</p><pre className="overflow-auto whitespace-pre-wrap text-xs">{JSON.stringify(q, null, 2)}</pre><Button variant="danger-ghost" onClick={() => { setActionError(''); setPendingDelete(q) }}>Артық сұрақты жою</Button></details>)}</Card>}
    </>}
  </div>
}
...
function QuestionEditor({ variantId, subject, slot, question, contextReady, onSaved, onDirty }: { variantId: number; subject: string; slot: Slot; question?: Question; contextReady: boolean; onSaved: () => Promise<void>; onDirty: (value: boolean) => void }) {
  ...
  return <Card className="p-5 sm:p-6"><form onSubmit={save} onChange={() => { onDirty(true); setSaved(false) }} className="space-y-5">
    ...
    {slot.context_start !== null && <details className="rounded-xl border border-border p-4" open={contextMode !== 'shared'}><summary className="cursor-pointer font-semibold">Осы сұрақтың контекстін жеке баптау</summary><div className="mt-4 space-y-3">
      ...
      <select aria-label="Контекстті қолдану тәсілі" className={inputClass} value={contextMode} onChange={e => setContextMode(e.target.value as typeof contextMode)}><option value="shared">Ортақ контекстті өзгеріссіз қолдану</option><option value="addendum">Ортақ контекстке мәтін қосу</option><option value="override">Тек осы сұрақта контекстті ауыстыру</option></select>
      {contextMode !== 'shared' && <textarea aria-label="Сұрақтың жеке контексті" className={inputClass} rows={4} maxLength={30000} required value={contextOverride} onChange={e => setContextOverride(e.target.value)} placeholder={contextMode === 'addendum' ? 'Ортақ контексттен кейін көрсетілетін қосымша мәтін' : 'Тек осы сұрақта көрсетілетін контекст'} />}
    ...
    <Card className="space-y-4 bg-surface-muted p-4"><div><h4 className="font-semibold">Сұрақ суреті (міндетті емес)</h4><p className="text-sm text-muted">Кез келген сұраққа JPG, PNG, GIF немесе WebP жүктеуге болады.</p></div>
      <input aria-label="Сұрақ суретін жүктеу" type="file" accept="image/png,image/jpeg,image/gif,image/webp" disabled={uploading} onChange={e => { void uploadImage(e.target.files?.[0]); e.currentTarget.value = '' }} />
      ...
      {previewUrl && <><img src={previewUrl} alt={imageAlt || 'Сұрақ суреті'} className="max-h-72 max-w-full rounded-xl object-contain" style={{ width: Math.min(imageWidth, 760) }} />
        ...
        <Button type="button" variant="danger-ghost" onClick={() => { if (!confirm('Суретті сұрақтан алып тастағыңыз келе ме?')) return; setImageFileId(null); setLegacyImageUrl(''); setImagePlacement('after'); setPrompt(v => v.replace('{{image}}', '')); onDirty(true) }}>Суретті сұрақтан алып тастау</Button></>}
    </Card>
    ...
    {message && <p role="alert" className="text-sm text-danger">{message}</p>}{saved && <p role="status" className="text-sm text-success-foreground">Сұрақ сақталды</p>}
