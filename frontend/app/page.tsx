...
const navItems = [
  ...
  ['Филиал', 'branch'], ['Байланыс', 'contact'],
] as const
...
function LandingHeader() {
  ...
  return <header className={styles.header}><div className={styles.headerInner}><a href="#top" aria-label="Басты бет"><Brand /></a><nav className={`${styles.nav} ${open ? styles.navOpen : ''}`}>{navItems.map(([label, id]) => <a key={id} href={`#${id}`} onClick={() => setOpen(false)}>{label}</a>)}<Link href="/login" onClick={() => setOpen(false)}>Кіру</Link><a className={styles.navCta} href="#contact" onClick={() => setOpen(false)}>Кеңес алу</a></nav><button className={styles.menu} onClick={() => setOpen(!open)} aria-label="Мәзірді ашу">{open ? <X /> : <Menu />}</button></div></header>
}
